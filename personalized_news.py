...
personalized_user = {
                      'Bala': 
                              {
                               'articles': [], 
                               'interests': ["startups","mergers and acquisition","basketball"]
                              },
                      'Sid': 
                             {
                               'articles': [], 
                               'interests': ["hip-hop","bitcoin","finance"]
                             }
                    }
...
def get_personalized_headlines(name):
       for interest in personalized_user[name]['interests']:
              headlines = newsapi.get_everything(q="+"+interest, language = 'en', page_size = 100, sort_by='relevancy')
              personalized_user[name]['articles'] += headlines['articles'][:3]
...
def create_welcome_goodbye(name):
       print('Creating welcome message')
       WELCOME_MESSAGE = 'Good Morning {0}! And welcome to your personalized morning minutes. Here are the top headlines for Thursday, January 30th'.format(name)
...
def main_template(name):
       get_personalized_headlines(name)
       urls, headlines = extract_minits(personalized_user[name], True)

       if name == 'Bala':
              receiverEmailList = ['karthikravi97@gmail.com']
       else:
              receiverEmailList = ["karthikravi97@gmail.com"]
       # with open(MAILING_LIST) as csvfile:
       #     receiverEmailList = list(csv.reader(csvfile))# change contents to float
       ...
       morning_minits += AudioSegment.from_mp3(GOODBYE_FILENAME)
       background_music = AudioSegment.from_mp3('newsmusic.mp3') + 100
       output = background_music.overlay(morning_minits, position=3000)
       output = truncate_audio(output, 0, 45)

       today = str(datetime.today().strftime('%m-%d-%Y'))

       outputFileName = str(name+'_todays_peronalized_minits'+ '.mp3')
       output.export(outputFileName, format='mp3')
